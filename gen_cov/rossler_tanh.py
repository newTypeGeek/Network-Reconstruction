...
def tanh_couple(W, state, N):
      ...
      \sum_{j \neq i} g_{ij} A_{ij} \tanh( state[j] - state[i] ) (Eq. (*))
...
def rossler_tanh(W, c1, c2, c3, sigma, int_dt, sample_dt, sample_start, data_num, get_ts=False):
    ...
    assert (type(c1) == int or type(c1) == float) and np.isfinite(c1) == True, "Parameter c1 must be a real number"
    assert (type(c2) == int or type(c2) == float) and np.isfinite(c2) == True, "Parameter c2 must be a real number"
    assert (type(c3) == int or type(c3) == float) and np.isfinite(c3) == True, "Parameter c3 must be a real number"
    ...
    for t in tqdm(range(T)):
        ...
        elif np.isnan(z).any() == True or np.isinf(z).any() == True:
            assert False, "The dynamics blows up!"
